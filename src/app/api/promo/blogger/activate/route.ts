...
// POST - активация промокода блогера пользователем
export async function POST(request: Request) {
  try {
    ...
    // Находим промокод блогера
    const promoCode = await prisma.bloggerPromoCode.findUnique({
    ...
    return NextResponse.json({
      ...
      data: {
        days: promoCode.days,
        bloggerUsername: promoCode.bloggerUsername,
        expiresAt,
      }
    })
  } catch (error) {
    console.error('Activate blogger promo code error:', error)
    return NextResponse.json(
