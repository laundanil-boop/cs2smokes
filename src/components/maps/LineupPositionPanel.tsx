...
export default function LineupPositionPanel({
  ...
  // Сбрасываем состояние при открытии новой позиции
  useEffect(() => {
    if (position) {
      setIsExpanded(false)
      setDragOffset(0)
  ...
  const handleTouchMove = (e: React.TouchEvent) => {
    ...
    // Разрешаем свайп только вниз и когда панель не раскрыта
  ...
  return (
    <>
      {/* Затемнение фона */}
      <div
        className="fixed inset-0 bg-black/50 backdrop-blur-sm z-40 md:block hidden"
        onClick={handleClose}
      />

      {/* Мобильная панель (bottom sheet) */}
      <div
        ref={panelRef}
        ...
        className={`
          fixed inset-x-0 bottom-0 md:inset-y-0 md:right-0 md:left-auto w-full md:max-w-md
          bg-gray-900/95 backdrop-blur-xl md:border-l border-gray-700 shadow-2xl z-50
      ...
      >
        {/* Индикатор свайпа (только мобильная версия) */}
        <div
          className="flex items-center justify-center py-3 cursor-pointer md:hidden"
